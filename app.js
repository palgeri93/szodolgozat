...
let API_BASE = "https://szodolgozat.onrender.com/";

// Auth/session
let teacherToken = null;
let activeStudentConfig = null; // amit a kódhoz letöltöttünk
let activeStudentCode = null;
...
let current = {
  ...
  responses: [], // [{prompt, answer, correct, point, mode}]
  index: 0,
  score: 0,
  locked: false,
  startTs: 0,
...
function startTimer() {
  ...
  timerHandle = setInterval(() => {
    const elapsedMs = Date.now() - current.startTs;
    el("timer").textContent = formatTime(elapsedMs);

    // időlimit: automatikus lezárás + mentés
    if (current.timeLimitSec && elapsedMs >= current.timeLimitSec * 1000) {
      finishQuiz(true);
    }
  }, 250);
}
...
function startQuiz() {
  ...
  // Tanulói mód: csak akkor induljon, ha betöltöttük a kód konfigurációját
  if (getRole() === "student") {
    const code = normalize(el("studentCodeInput").value);
    if (!code) {
      showStatus("Kérlek, add meg a tanulói kódot.", "error");
      return;
    }
    if (!activeStudentConfig || activeStudentCode !== code) {
      showStatus("Előbb töltsd be a kód beállításait (Kód betöltése).", "error");
      return;
    }
  }

  // Teacher által rögzített config (student módban kötelező)
  const sheet = el("gradeSelect").value;
  const lesson = el("lessonSelect").value;
  ...
  current = {
    ...
    locked: false,
    startTs: 0,
    timeLimitSec: activeStudentConfig?.timeLimitSec ?? null,
  };
...
async function submitResultsToServer(rec) {
  // csak tanulói módban küldünk (tanár módban lehet "próba", azt nem mentjük)
  if (getRole() !== "student") return;
  const code = activeStudentCode;
  if (!code) return;
...
function finishQuiz(endedByTimeout = false) {
  // ha már vége, ne fussunk kétszer
  ...
  // UI lezárás
  el("prompt").textContent = endedByTimeout
    ? `Lejárt az idő! Eredmény: ${current.score} / ${current.questions.length}`
    : `Vége! Eredmény: ${current.score} / ${current.questions.length}`;
  ...
  // async mentés szerverre
  submitResultsToServer(rec)
    .then(() => {
      setFeedback(
        `Mentve (szerver): ${rec.score}/${rec.total} • idő: ${formatTime(rec.seconds * 1000)} • ${rec.when}`,
        true
      );
    })
    .catch((e) => {
      setFeedback(
        `⚠️ Nem sikerült a szerverre menteni: ${e.message}. (A helyi mentés megmaradt ezen a gépen.)`,
        false
      );
    });
}
...
function setRole(role) {
  localStorage.setItem("role_v1", role);
  applyRoleUI();
}

function getRole() {
  return localStorage.getItem("role_v1") || "student";
}
...
function applyRoleUI() {
  ...
  if (role === "teacher") {
    ...
    roleTeacherBtn.className = "px-4 py-2 rounded-xl font-semibold bg-slate-900 text-white";
    roleStudentBtn.className = "px-4 py-2 rounded-xl font-semibold bg-slate-200 hover:bg-slate-300";
    // tanári módban ne kérjünk kódot
    el("studentCodeInput").value = "";
  } else {
    teacherPanel.classList.add("hidden");
    roleStudentBtn.className = "px-4 py-2 rounded-xl font-semibold bg-slate-900 text-white";
    roleTeacherBtn.className = "px-4 py-2 rounded-xl font-semibold bg-slate-200 hover:bg-slate-300";
  }
}

function lockStudentConfigUI(locked) {
  // tanuló ne tudja átállítani
  [
    "gradeSelect",
    "lessonSelect",
    "rangeFrom",
    ...
    if (locked) node.classList.add("opacity-70");
    else node.classList.remove("opacity-70");
  });
}

async function loadConfigForStudentCode() {
  const code = normalize(el("studentCodeInput").value);
  if (!code) {
    showStatus("Add meg a tanulói kódot.", "error");
    return;
  }
  try {
    const cfg = await apiFetch(`/api/student/config?code=${encodeURIComponent(code)}`, { method: "GET" });
    activeStudentConfig = cfg;
    activeStudentCode = code;

    // beállítjuk a UI-t a kapott config alapján
    if (cfg.sheet) el("gradeSelect").value = cfg.sheet;
    populateLessons(el("gradeSelect").value);
    if (cfg.lesson) el("lessonSelect").value = cfg.lesson;
    refreshLessonStatsAndDefaults();
    if (cfg.rangeFrom) el("rangeFrom").value = String(cfg.rangeFrom);
    if (cfg.rangeTo) el("rangeTo").value = String(cfg.rangeTo);
    if (cfg.mode) el("modeSelect").value = cfg.mode;
    el("noRepeatToggle").checked = !!cfg.noRepeat;
    if (cfg.count) el("countInput").value = String(cfg.count);

    lockStudentConfigUI(true);
    showStatus(`Kód betöltve: ${code}. Indíthatod a dolgozatot.`, "ok");
  } catch (e) {
    activeStudentConfig = null;
    activeStudentCode = null;
    lockStudentConfigUI(false);
    showStatus(`Nem sikerült betölteni a kódot: ${e.message}`, "error");
  }
}

async function teacherLogin() {
  const pwd = el("teacherPassword").value;
  if (!pwd) {
    showStatus("Add meg a tanári jelszót.", "error");
    return;
  }
  try {
    const out = await apiFetch("/api/teacher/login", {
      method: "POST",
      body: JSON.stringify({ password: pwd }),
    });
    teacherToken = out.token;
    el("teacherAuthed").classList.remove("hidden");
    showStatus("Sikeres tanári belépés.", "ok");
  } catch (e) {
    teacherToken = null;
    el("teacherAuthed").classList.add("hidden");
    showStatus(`Sikertelen belépés: ${e.message}`, "error");
  }
}
...
function genStudentCode() {
  ...
  el("studentCode").value = s;
}

async function publishTeacherSession() {
  if (!teacherToken) {
    showStatus("Előbb lépj be tanárként.", "error");
    return;
  }
  const code = normalize(el("studentCode").value);
  if (!code) {
    showStatus("Adj meg (vagy generálj) tanulói kódot.", "error");
    return;
  }
  ...
  const noRepeat = el("noRepeatToggle").checked;
  const count = Math.max(1, parseInt(el("countInput").value || "10", 10));
  const rangeFrom = Math.max(1, parseInt(el("rangeFrom").value || "1", 10));
  const rangeTo = Math.max(1, parseInt(el("rangeTo").value || "1", 10));
  const timeLimitMin = Math.max(1, parseInt(el("timeLimitMin").value || "5", 10));
  ...
  try {
    await apiFetch("/api/teacher/session", {
      method: "POST",
      body: JSON.stringify({ token: teacherToken, code, config: cfg }),
    });
    showStatus(`Beállítások mentve ehhez a kódhoz: ${code}`, "ok");
  } catch (e) {
    showStatus(`Nem sikerült menteni: ${e.message}`, "error");
  }
}
...
function downloadResultsXlsx() {
  ...
  // fájlletöltés: sima navigáció
  window.open(apiUrl(`/api/teacher/export.xlsx?token=${encodeURIComponent(teacherToken)}`), "_blank");
}

function wireUI() {
  // API base input
  const savedApi = localStorage.getItem("api_base_v1");
  if (savedApi) API_BASE = savedApi;
  el("apiBaseInput").value = API_BASE;
  el("apiBaseInput").addEventListener("change", () => {
    API_BASE = normalize(el("apiBaseInput").value) || API_BASE;
    localStorage.setItem("api_base_v1", API_BASE);
  });

  // role
  el("roleStudentBtn").addEventListener("click", () => setRole("student"));
  el("roleTeacherBtn").addEventListener("click", () => setRole("teacher"));
  applyRoleUI();

  // teacher UI
  el("teacherLoginBtn").addEventListener("click", teacherLogin);
  el("genCodeBtn").addEventListener("click", genStudentCode);
  el("publishSessionBtn").addEventListener("click", publishTeacherSession);
  el("downloadXlsxBtn").addEventListener("click", downloadResultsXlsx);

  // student config load
  el("loadConfigBtn").addEventListener("click", loadConfigForStudentCode);
